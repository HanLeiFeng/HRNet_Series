...
from .attention import se_block, cbam_block, eca_block, CA_Block, SpatialAttention
...
class HRnet(nn.Module):
    def __init__(self, num_classes = 21, backbone = 'hrnetv2_w18', pretrained = False, phi=3):
        ...
        if phi >=1 and phi <=4:
            ...
            self.sp_attetion = SpatialAttention(7)
    ...
    def forward(self, inputs):
        ...
        y3 = F.interpolate(x3, size=(x2.size(2), x2.size(3)), mode='bicubic', align_corners=True)
        y3 = self.sp_attetion(y3)
        x2 = torch.cat([x[2], y3], 1)
        x2 = self.feat_attetion_fuse32(x2)
        y2 = F.interpolate(x2, size=(x1.size(2), x1.size(3)), mode='bicubic', align_corners=True)
        y2 = self.sp_attetion(y2)
        x1 = torch.cat([x[1], y2], 1)
        x1 = self.feat_attetion_fuse321(x1)
        y1 = F.interpolate(x1, size=(x0.size(2), x0.size(3)), mode='bicubic', align_corners=True)
        y1 = self.sp_attetion(y1)
        x0 = torch.cat([x[0], y1], 1)
