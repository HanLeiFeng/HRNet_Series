...
class SegFormer(nn.Module):
    def __init__(self, num_classes = 21, phi = 'b0', pretrained = False):
        ...
        self.STL = STL(64, 768)
        self.conv = nn.Conv2d(768*4, 128, 1, 1, 0, bias=False)
        self.bn = nn.BatchNorm2d(128, momentum=0.0003)
        self.relu = nn.ReLU(inplace=True)
        self.linear_pred = nn.Conv2d(128, num_classes, kernel_size=1)
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    def forward(self, inputs):
        inputs = inputs.to(self.device)
        H, W = inputs.size(2), inputs.size(3)
        x = self.backbone.forward(inputs)
        # print("输入图片经过SegFormer_b3_BackBone网络模型后输出的的形状：", x[0].shape)
        x_oux = self.STL(x[0])
        # print("输入图片经过STL模块后网络模型后输出的的形状：", x_oux.shape)
        x = self.decode_head.forward(x)
        # print("输入图片经过SegFormer_b3_BackBone_MLP网络模型后输出的的形状：", x.shape)
        x = torch.cat([x, x_oux],dim=1)
        # print("将resnet50与SegFormer_b3网络的输出进行通道拼接后的形状：", x.shape)
        x = self.conv(x)
        x = self.bn(x)
        x = self.relu(x)
        # print("x.shape_2")
