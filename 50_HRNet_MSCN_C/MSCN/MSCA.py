...
with open('MSCN/config.yaml') as fh:
...
from MSCN.bricks import DownSample, LayerScale, StochasticDepth, DWConv3x3, NormLayer
...
# model = MSCANet(in_channnels=384, embed_dims=[384, 384, 460, 256],
#                 ffn_ratios=[4, 4, 4, 4], depths=[12, 3],
#                 num_stages=1, ls_init_val=1e-2, drop_path=0.0)
# summary(model, (3,1024,2048))


# y = torch.randn(2, 384, 16, 16)  # .to('cuda' if torch.cuda.is_available() else 'cpu')
# x = model.forward(y)
#
# for i in range(1):
#     print(x[i].shape)
