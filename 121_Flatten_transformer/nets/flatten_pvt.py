...
def _cfg(url='', **kwargs):
    return {
        'url': url,
        'num_classes': 21, 'input_size': (3, 512, 1024), 'pool_size': None,
...
class FocusedLinearAttention(nn.Module):
    def __init__(self, dim, resolution, idx, split_size=7, dim_out=None, num_heads=8, attn_drop=0., proj_drop=0.,
        ...
        # print('Linear Attention H_sp:{}xW_sp:{} focusing_factor:{} kernel_size:{}'.
        #       format(H_sp, W_sp, focusing_factor, kernel_size))

    def im2cswin(self, x):
        B, N, C = x.shape
        # print("182===》输入大小：", x.shape)
        H = int(np.sqrt(N//2))
        W= 2 * H
        # print("184===》图像长度H=：", H)
        # print("185===》图像宽度W=：", W)
        x = x.transpose(-2, -1).contiguous().view(B, C, H, W)
        # print("187===》调整通道为：(B,C,H*W)并变换为(B,C,H,W)：",x.shape )
        # print("188===》窗口长度H=：", self.H_sp)
        # print("189===》窗口宽度W=：", self.W_sp)
        x = img2windows(x, self.H_sp, self.W_sp)
...
def img2windows(img, H_sp, W_sp):
    ...
    """
    B, C, H, W = img.shape
    # print("349->原图像的形状为:", img.shape)
    # print("350->窗口块的长度为H_sp=：", H_sp)
    # print("351->窗口块的宽度为W_sp=：", W_sp)
    img_reshape = img.view(B, C, H // H_sp, H_sp, W // W_sp, W_sp)
    # print("353->划分后的形状为（B, C, H // H_sp, H_sp, W // W_sp, W_sp）：", img_reshape.shape)
    img_temp = img_reshape.permute(0, 2, 4, 3, 5, 1).contiguous()
    img_perm   = img_temp.reshape(-1, H_sp * W_sp, C)
    # print("356->调整通道后的形状为（B, H // H_sp, W // W_sp, H_sp, W_sp, C）：", img_temp.shape)
    # print("357->划分后的形状为（B*(H // H_sp)*(W // W_sp), H_sp*W_sp, C）：", img_perm.shape)
    return img_perm
...
class CSWinTransformer(nn.Module):
    ...
    def __init__(self, img_size=[512,1024], patch_size=16, in_chans=3, num_classes=19, embed_dim=96, depth=[2, 2, 6, 2],
                 split_size=[1, 2, 8, 8], la_split_size='1-2-8-8',
        ...
        '(2,64,512,512)->(2,64,128,256)->(2,32768,64)'
        curr_dim = embed_dim
        ...
        self.stage1 = nn.ModuleList([
                                        CSWinBlock(
                                            dim=curr_dim, num_heads=heads[0], reso=img_size[0] // 4, mlp_ratio=mlp_ratio,
        ...
        self.stage2 = nn.ModuleList(
            [CSWinBlock(
                dim=curr_dim, num_heads=heads[1], reso=img_size[0] // 8, mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias, qk_scale=qk_scale,
                split_size=split_sizes[i],
                drop=drop_rate, attn_drop=attn_drop_rate,
        ...
        temp_stage3.extend(
            [CSWinBlock(
                dim=curr_dim, num_heads=heads[2], reso=img_size[0] // 16, mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias, qk_scale=qk_scale,
                split_size=split_sizes[i],
                drop=drop_rate, attn_drop=attn_drop_rate,
        ...
        self.stage4 = nn.ModuleList(
            [CSWinBlock(
                dim=curr_dim, num_heads=heads[3], reso=img_size[0] // 32, mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias, qk_scale=qk_scale,
                split_size=split_sizes[i],
                drop=drop_rate, attn_drop=attn_drop_rate,
        ...
        # self.linear_pred = nn.Conv2d(curr_dim, num_classes, kernel_size=1)
    ...
    def forward_features(self, x):
        ...
        # print("one507forward_features:", x.shape)
        for blk in self.stage1:
            if self.use_chk:
                x = checkpoint.checkpoint(blk, x)
            else:
                x = blk(x)
        # print("one513forward_features:", x.shape)
        for pre, blocks in zip([self.merge1, self.merge2, self.merge3],
                               [self.stage2, self.stage3, self.stage4]):
            x = pre(x)
            # print("one517forward_features:", x.shape)
            for blk in blocks:
                if self.use_chk:
                    x = checkpoint.checkpoint(blk, x)
                else:
                    x = blk(x)
        # print("one522forward_features:", x.shape)
        b, wh, c = x.size(0), x.size(1), x.size(2)
        h = int((wh // 2) ** 0.5)
        w = int((wh // h))
        x = self.norm(x)
        x_reshaped = x.reshape(b, w, h, c)
...
# def FLatten_CSWin_64_24181_tiny_224(pretrained=True, num_classes=21, **kwargs):
#     model = CSWinTransformer(patch_size=4, num_classes=num_classes, embed_dim=64, depth=[2, 4, 18, 1],
...
#         model.load_state_dict(torch.load("model_data/ckpt_pvt_t.pth"), strict=False)
#     return model
...
def FLatten_CSWin_64_24181_tiny_224(pretrained=True, num_classes=21, **kwargs):
    ...
    if pretrained:
        print("Load backbone weights")
        model.load_state_dict(torch.load("model_data/ckpt_pvt_t.pth"), strict=True)
    return model
