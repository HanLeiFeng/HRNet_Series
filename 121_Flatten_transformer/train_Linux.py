...
from nets.Flatten import SegFormer0
...
if __name__ == "__main__":
    ...
    #----------------------------------------------------------------------------------------------------------------------------#
    # model_path      = "/root/autodl-tmp/01_Projects/19_segformer/logs/last_epoch_weights.pth"
    model_path = ""
    #------------------------------#
    #   输入图片的大小
    #------------------------------#
    ...
    #------------------------------------------------------------------#
    # save_dir            = '/root/autodl-tmp/01_Projects/01_segformer_b3_c/logs'
    save_dir = 'logs'
    #------------------------------------------------------------------#
    ...
    #------------------------------------------------------------------#
    # VOCdevkit_path  = '/root/autodl-tmp/01_Datasets/02_CityScapes'
    VOCdevkit_path = 'E:/01_DataSets/04_CItyScapes_mini'
    #------------------------------------------------------------------#
    #   建议选项：
    #   种类少（几类）时，设置为True
    ...
    # with open("/root/autodl-tmp/01_Datasets/02_CityScapes/class_weights.pkl", "rb") as file: # (needed for python3)
    #     cls_weights = np.array(pickle.load(file))
    # cls_weights = cls_weights.astype('float32')
    with open("E:/01_DataSets/04_CItyScapes_mini/class_weights.pkl", "rb") as file:  # (needed for python3)
        cls_weights = np.array(pickle.load(file))
    cls_weights = cls_weights.astype('float32')
    #------------------------------------------------------------------#
    ...
    model   = SegFormer0(num_classes=num_classes, pretrained=pretrained)
    if not pretrained:
        weights_init(model)
    if model_path != '':
        #------------------------------------------------------#
