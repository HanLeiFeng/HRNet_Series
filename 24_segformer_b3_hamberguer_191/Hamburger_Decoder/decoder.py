...
class HamDecoder(nn.Module):
    '''SegNext'''
    def __init__(self, outChannels, config, enc_embed_dims):
        super().__init__()

        ham_channels = config['ham_channels']
        self.squeeze = ConvRelu(enc_embed_dims, ham_channels)
        self.ham_attn = HamBurger(ham_channels, config)
        self.align = ConvRelu(ham_channels, outChannels)
       
    def forward(self, x):
        # features = features[0:] # drop stage 1 features b/c low level
