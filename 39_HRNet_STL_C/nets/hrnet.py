...
class HRnet(nn.Module):
    def __init__(self, num_classes = 21, backbone = 'hrnetv2_w18', pretrained = False):
        ...
        last_inp_channels   = int(np.sum(self.backbone.model.pre_stage_channels) + 64)
    ...
    def forward(self, inputs`):
        ...
        x, x_aux = self.backbone(inputs)

        x_aux = self.STL(x_aux)
        ...
        x = torch.cat([x[0], x1, x2, x3, x_aux], 1)
